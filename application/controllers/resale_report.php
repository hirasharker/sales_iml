...
class Resale_Report extends CI_Controller {
	...
	public function general_resale_report(){
		...
		$resale_data['seize_list']							=	$this->seize_model->get_all_seizes();

		$resale_data['seize_depot_list']						=	$this->seize_model->get_all_seize_depots();
		
		$resale_data['employee_list']						=	$this->employee_model->get_all_employees();

		$resale_data['zone_list']							=	$this->zone_model->get_all_zones();

		$report_data['navigation'] 							=   $this->load->view('template/navigation','',TRUE);
        $report_data['content']								=	$this->load->view('pages/report/resale_report',$resale_data,TRUE);
        $report_data['footer']     							=   $this->load->view('template/footer','',TRUE);
		
		$this->load->view('template/main_template',$report_data);
	}

	public function generate_resale_report(){
		$report_data										=	array();
		$resale_data											=	array();
		...
		$resale_data['resale_list']					=	$this->resale_model->get_all_resale_data_by_search_criteria($zone_id, $rm_id, $zm_id, $depot_id, $status, $start_date, $end_date);

		// echo json_encode($zone_id); exit();
		$resale_data['end_date']								=	$end_date;

		$resale_data['employee_list']						=	$this->employee_model->get_all_employees();
		$resale_data['zone_list']							=	$this->zone_model->get_all_zones();

        $report_data['content']								=	$this->load->view('pages/report/resale_report_table',$resale_data,TRUE);
